...
combine_truth_tables <- function(list_truth_tables, maximum_number_input_variables) {
  ...
  y_columns <- lapply(list_truth_tables, function(truth_table_matrix) {
    ...
    # Find common columns between the combined truth-table und the current truth-table
    ...
    y_column <- apply(combined_truth_table_matrix[, common_columns, drop = FALSE], 1, function(row) {
      ...
      # Wenn eine Übereinstimmung gefunden wurde, den y-Wert zurückgeben, andernfalls NA
    ...
    # Rückgabe des y-Vektors
    return(y_column)
  })
  
  # Binde die y-Spalten an die kombinierte Wahrheitstabelle an
  combined_truth_table_matrix <- cbind(combined_truth_table_matrix, do.call(cbind, y_columns))
  
  # Setze die Namen für die y-Spalten
  ...
  # Rückgabe der kombinierten Wahrheitstabelle mit den hinzugefügten y-Spalten
...
result_truth_table_2 <- generate_truth_table(3,5)
